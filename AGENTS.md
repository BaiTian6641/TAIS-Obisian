...
- `src/tais_obsidian/`：框架包（`uv pip install -e .` 后可 import）
  ...
  - `runtime/`：运行时服务（`pagetable.py` 页表 SQLite、`blockstore.py` 块存储 usage_weighted、`pager.py` 缺页 fail-closed、`bus.py` Memory Bus、`ca1_gate.py` **自适应**巩固门 v1.1（RE_VERIFY 边缘带+证据加权+信源可信度在线学习）、`ca3_ppr.py` 联想、`state_ckpt.py` GDN 状态持久化、`safety.py` 安全管线）
...
- `configs/`：`pilot_0p1b.json`（hybrid 基线）、`pilot_0p1b_pm.json`（PM-stream 消融）、`pilot_0p1b_tri.json`（三级栈消融）、`pilot_0p1b_pmtri.json`（组合）；**`pilot_0p1b_attn.json`（attn_only 孪生）已于 2026-07 移除（对照组废弃）**；`pilot_0p5b_gdn2.json`（0.5B GDN-2：512.8M d1280×24）、`pilot_1b_gdn2.json`（**1B GDN-2 预训练**：1017.7M d1536×32 层=8×{G2G2G2A}，Muon，max_steps 38000 含中训练口径，Colab 执行）
- `scripts/`：`check_env.py`（环境自检）、`prepare_data.py`（FineWeb-Edu → 训 32k BPE → 120M tokens shards）、`prepare_data_0p5b.py`（**0.5B 3B tokens 多领域混合**：fineweb_edu 70%/NuminaMath 15%/cosmopedia 10%/FineWeb2-HQ 中文 5%，断流重试加固，输出 data/shards_0p5b）、`train_dp.py`（**双卡手动 DP**：WorkerNode 线程化+时间均衡 accum，3.1k tok/s +24%，Windows 无 NCCL 单进程实现）、`smoke_overfit.py` / `smoke_overfit_pm.py` / `smoke_overfit_tri.py`（三变体过拟合冒烟）、`extend_tokenizer.py`（E+-2 特殊 token 扩容，幂等）、`kal_probe.py`（E+-3 KAL 探针管线，输出 `runs/kal_probe/report.json`）、`kal_truth_finetune_v2.py`（**P1 校准 v2**：锚集扩充 AUROC 0.845/0.829 双口径达标；预测反馈循环诚实负结果）、`prepare_data_1b.py`（**1B 10B tokens 四源流式混合**：fineweb_edu-100BT 73%/math(NuminaMath+FineMath-4+)12%/cosmopedia 10%/中文 5%，断点续跑+原子写+max_id 全量扫描，输出 data/shards_1b）、`extend_context.py`（**渐进扩窗**：RoPE 缓存扩容至 256K + YaRN scaling + 阶段课程微调 + `--resume` 断点续训，复用 train.py 组件）、`bench_long_seq_cost.py`（长 seq 成本实测：CSA/HCA 打分随 T²、滑窗 math-SDPA 显存随 T²）、`interactive_chat.py` + `interactive_validation_demo.py`（**交互式全链验证**：REPL 对话/教学/探针/睡眠固化 + 确定性四阶段剧本，产物 runs/interactive_validation/）、`train_manifold_projector.py`（**流形投影器训练**：冻结主干 VICReg+共形等距，聚簇 1.989/等距 0.977，sidecar projector.pt）、`manifold_preview.py`（**流形推理预览**：逐生成步 3D 轨迹渲染+知识块叠加+坏路径检测）、`extended_validation.py`（**五场景扩展测试**：S1 知识推理/S2 教学重试/S3 桥接/S4 动态词表/S5 睡眠增强，113 轮日志 14/15 判据过）、`export_final.py`（**latest.pt → save_pretrained 导出**：从 ckpt["model_cfg"] 还原结构 + 复制 tokenizer.json，HF 上传/下载推理链路）、`train_manifold_projector.py`（**思考流形投影器训练**：冻结主干逐位校验 + 多层 hidden 段表征 + 语义步长 1−cos 自监督，复用 manifold.py 共形等距+VICReg 损失，sidecar 存 checkpoints/pilot_0p1b_gdn2_10k_unified_manifold/projector.pt）、`manifold_preview.py`（**流形推理预览**：逐生成步 ℓ10 → project_3d 3D/2D 三视图渲染，Carbon 色板，KAL certainty 坏路径四类检测，知识块叠加 + 随机|已训 --compare 对比，产物 runs/manifold_preview/）
- `tests/`（**471 项 pytest 全绿**，2026-07-31）：第一阶段 M0–M8 测试（test_gdn/cache/capture/blockpath/pmstream/tri_attention/tri_indexer/kal/tokenizer_ext/tais_kernel/kernel_wiring/kernel_route_candidates/hrl/hrl_init/gdn2_indexer/runtime/injection/sleep/dyn_vocab/safety）+ 第二阶段（test_manifold/manifold_bridge/thought_core/reasoning_loop/cot_projection/thought_visualizer/path_integration/thinking_e2e/thinking_real_adapter/thought_core_integration）+ 主动求知（test_inquiry_branch/inquiry_executor/inquiry_consolidation/active_inquiry_full_chain）+ 知识内化（test_teaching_data/internalization_e2e/retrieval_recall/gated_fusion/kaplan_extract/unified_checkpoint/memlayer_internalization）+ 门控自适应（test_decoupled_gate/fully_decoupled/niah_length_scan）+ 优化（test_muon/set_lr_wsd）+ 校准（test_kal_calibration_v2）+ 链路回归（test_resume_e2e/save_load_sizes/generate_guards/export_final/rope_extension/interactive_validation）+ CA1 自适应（test_ca1_adaptive）+ 流形（test_manifold_preview）+ 扩展场景（test_extended_validation）
...
- 单元测试：`python -m pytest tests/ -q`（471 项）
- 训练：`python -m tais_obsidian.train --config configs/<cfg>.json`（续训加 `--resume checkpoints/<run>/latest.pt`；长任务加 `python -u` 防输出缓冲；结束时 final/ 随附 tokenizer.json）
- 导出：`python scripts/export_final.py --ckpt checkpoints/<run>/latest.pt --out checkpoints/<run>/final`（latest.pt → save_pretrained + tokenizer.json，HF 上传/下载推理链路）
- 推理：`python -m tais_obsidian.generate --ckpt checkpoints/<run>/final --prompt "..."`（--tokenizer 缺省依次尝试 `<ckpt>/tokenizer.json` → `data/tokenizer/tokenizer.json`）
...
- **M0–M8 里程碑链**（《部件实现详细计划》§IV，每级有退出标准，不达标不进下一级）：
  ...
  - **当前阶段**：**Colab 1B 训练（当前主线，2026-07-31 转向）**——1017.7M 配置 + 10B 数据脚本 + 全链路审阅修复 + `notebooks/TAIS_1B_Colab.ipynb` 就绪，详见 `docs/Colab_1B训练计划.md`；P1 校准 ✅ 达标（0.845/0.829）；**运行时全链交互验证完成**（S1–S5 扩展测试 14/15 判据过：桥接推理增强实证、CA1 自适应 v1.1 根治信源边缘效应、流形投影器已训 sidecar；诚实负结果=OOV 新词注入召回失败→载体分布边界，列 1B 首要复测）；RoPE 扩容初版已落地（256K 渐进扩窗待 1B 之后）；后续 ④ 记忆层读出/寻址接口训练 ⑤ lm-eval 生态接入 ⑥ 1.5B 扩展规划。
