...
- 单元测试：`python -m pytest tests/ -q`（443 项）
...
- **M0–M8 里程碑链**（《部件实现详细计划》§IV，每级有退出标准，不达标不进下一级）：
  ...
  - **当前阶段**：**Colab 1B 训练（当前主线，2026-07-31 转向）**——1017.7M 配置 + 10B 数据脚本 + 全链路审阅修复（437 测试绿）+ `notebooks/TAIS_1B_Colab.ipynb` 就绪，详见 `docs/Colab_1B训练计划.md`；P1 校准 ✅ 达标（0.845/0.829）；RoPE 扩容初版已落地（rope_scaling none/yarn + extend_context.py + test_rope_extension 绿；256K 渐进扩窗待 1B 之后执行，方案见 MEMORY.md 任务②）；后续 ④ 记忆层读出/寻址接口训练 ⑤ lm-eval 生态接入（auto_map/PreTrainedModel 化）⑥ 1.5B 扩展规划。
