...
| 信号质量 | 自报置信度（不可靠）、token 概率（受污染） | 内部状态探针（SAPLMA 71–83% **准确率**（非 AUROC，v2.5 勘误），优于输出分布） |
...
2. **内部诚实 ≠ 口头诚实，且后训练会压垮它**：SAPLMA（71–83% **准确率**，非 AUROC，v2.5 勘误）与 ITI（32.5%→65.1%）证明真实度信息存在于内部状态；"Uncertainty Collapse"（2026）进一步指出"我不知道"是**学来的、在后训练目标下脆弱**的行为——这既是 KAL 的动机，也是风险：三元奖励必须持续压制"自信编造"的吸引子。
...
词表是全框架最后满足"体外定义、不可微、部署后永不更新"三条的部件。痛点有文献级证据：① 新语义无槽位，反复碎片化拼装；② 碎片化有真实成本——SuperBPE（跨空白 superword，arXiv:2503.13423，COLM 2025 camera-ready，v2.5 补出处）同词表大小 30 任务平均 +4.0%（MMLU +8.2%）、推理省 27% 算力；③ 词表是被浪费的稀疏参数维度——Over-Tokenized Transformer（ICML 2025）发现**输入词表大小与训练 loss 呈 log-linear**，400M 靠 128× 输入词表追平 1B 基线，且输入/输出词表可解耦（输出扩张对小模型有害）。
